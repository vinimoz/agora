  ...
  getters: {
    ...
    getFamilies(): OptionFamily[] {
      ...
      return Object.values(familiesMap)
        ...
        .map((family) => ({
          ...family,
          types: (family.types || []).sort((a, b) => (a.sortOrder || 999) - (b.sortOrder || 999)),
        }))
    },

    getFamiliesWithTypes(): Array<
      OptionFamily & {
    ...
    getOptionTypeInfo:
      () =>
      (typeKey: string): InquiryOptionType | undefined => {
        const sessionStore = useSessionStore()
    ...
    isAllowedChildType:
      (state) =>
      (parentType: string, childType: string): boolean => {
    ...
    totalSupports(): number {
      return this.options.reduce((total, option) => total + (option.status?.countSupports || 0), 0)
    },

    totalComments(): number {
      return this.options.reduce((total, option) => total + (option.status?.countComments || 0), 0)
    },
    ...
    getOptionsByTargetId:
      (state) =>
      (targetId: number): Option[] =>
        state.options.filter((option) => option.targetId === targetId),
    ...
    userCreatedOptions(): Option[] {
      const sessionStore = useSessionStore()
      return this.options.filter((option) => option.owner?.id === sessionStore.currentUser?.id)
    },
    ...
    getFamilyColor:
      () =>
      (familyKey: string): string =>
        getFamilyColor(familyKey),

    getFamilyIcon:
      () =>
      (familyKey: string): string =>
        getFamilyIconName(familyKey),

    getFamilyIconComponent:
      () =>
      (familyKey: string): Component =>
        getFamilyIconComponent(familyKey),
      ...
      if (optionId) {
        ...
        if (option && option.status) {
          const count = commentsStore.comments.filter(
            (comment) =>
              comment.inquiryId === option.targetId &&
    ...
    async add(payload: {
      ...
      if (payload.parentId) {
        ...
        if (parentOption) {
          ...
          if (
            parentTypeInfo?.allowed_child_types &&
      ...
      return this.families
        .map((family) => {
          ...
          return {
            ...
            totalSupports: familyOptions.reduce(
              (total, option) => total + (option.status?.countSupports || 0),
              0
            ),
            totalComments: familyOptions.reduce(
              (total, option) => total + (option.status?.countComments || 0),
              0
            ),
            types: Object.entries(typeCounts).map(([type, count]) => ({
              type,
              name:
                this.getOptionTypeInfo(type)?.name || this.getOptionTypeInfo(type)?.label || type,
              count,
            })),
          }
        })
