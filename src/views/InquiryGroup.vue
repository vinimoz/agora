...
const displayedGroups = computed(() => {
  ...
  if (hasSlug.value && currentInquiryGroup.value) {
    const childGroups = inquiryGroupsStore.inquiryGroups.filter(
      (group) =>
        group.parentId === currentInquiryGroup.value?.id && 
        group.status?.groupStatus !== 'archived'
    )
...
const totalInquiries = computed(() => {
  ...
  return currentInquiryGroup.value.inquiryIds?.length || 0
})
...
const currentDescription = computed(() => {
  if (hasSlug.value && currentInquiryGroup.value) {
    return currentInquiryGroup.value.titleExt || 
           currentInquiryGroup.value.configuration?.description || 
           selectedTypeDescription.value
  }
...
function inquiryGroupAdded(newGroup: InquiryGroup) {
  ...
  if (newGroup.slug) {
    ...
    router.push({ name: 'group', params: { id: String(newGroup.id) } })
  }
}
...
const availableGroupTypesWithGroups = computed(() => {
  ...
  return types.filter((type: InquiryGroupType) => {
    ...
    const count = inquiryGroupsStore.inquiryGroups.filter(
      (g) => g.type === typeKey && g.parentId === null && g.status?.groupStatus !== 'archived'
    ).length
    return count > 0
  })
})
...
if (hasSlug.value) {
  ...
  const group = inquiryGroupsStore.bySlug(slug)
  if (group) {
    await inquiryGroupStore.load(group.id)
  } else {
