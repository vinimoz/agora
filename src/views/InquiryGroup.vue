  ...
  <NcAppContent class="inquiry-group-page">
    <div class="inquiry-group-page">
      ...
      <template v-else>
		...
		<div class="header-right">
			<ExperienceSwitcher
					v-if="currentInquiryGroup && hasCustomArchitecture"
	...
	<div v-else class="content-area">
		...
		<template v-else-if=" currentInquiryGroup">
			...
			<!-- Groups Grid -->
			<div v-if="hasSlug && displayedGroups.length > 0" class="groups-grid">
				<div 
		...
		<template v-else-if="hasSlug && currentInquiryGroup && displayedGroups.length === 0">
			<div class="empty-state">
				<div class="empty-icon">📁</div>
				<h3>{{ t('agora', 'No groups available') }}</h3>
				<p>{{ t('agora', 'There are no groups in this section yet.') }}</p>
				<NcButton v-if="canUserEditGroup(currentInquiryGroup)" type="primary" @click="createInquiryGroup(currentGroupType)">
		...
		<template v-else-if="!hasSlug">
			...
			<div v-else class="empty-state">
				...
				<h3>{{ t('agora', 'No groups available') }}</h3>
				<p>{{ t('agora', 'There are no groups in this section yet.') }}</p>
				<NcButton v-if="canUserEditGroup(null)" type="primary" @click="createInquiryGroup(currentGroupType)">
				{{ t('agora', 'Create first group') }}
				</NcButton>
			</div>
		</template>
...
const selectedTypeLabel = computed(() => 
    selectedTypeData.value?.label ? t('agora', selectedTypeData.value.label) : t('agora', 'You should not be here')
)
