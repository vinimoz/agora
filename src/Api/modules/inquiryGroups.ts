...
const inquiryGroups = {
  getInquiryGroups(): Promise<
    AxiosResponse<{ inquiryGroups: InquiryGroup[] }>
    > {
  ...
  addInquiryToGroup(
    ...
    if (inquiryGroupId) {
      ...
      throw new Error(
        'You must provide either a inquiryGroupId or a inquiryGroupName'
      );
    }
  ...
  removeInquiryFromGroup(
    ...
    AxiosResponse<{ inquiryGroup: InquiryGroup | null; inquiry: Inquiry }>
  > {
    return httpInstance.request({
      method: 'DELETE',
      url: `inquirygroup/${inquiryGroupId}/inquiry/${inquiryId}`,
      cancelToken:
        cancelTokenHandlerObject[
          this.removeInquiryFromGroup.name
        ].handleRequestCancellation().token
    });
  },
