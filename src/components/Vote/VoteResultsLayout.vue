            ...
            <table>
                ...
                <tbody>
                    <tr v-for="(item, index) in rankedItems" :key="item.id">
                        ...
                        <td class="votes-cell">{{ getItemVoteCount(item.id) }}</td>
                        <td class="percentage-cell">
                            <div class="mini-progress">
                                <div class="mini-progress-fill" :style="{ width: getPercentage(item) + '%' }" />
                                <span>{{ getPercentage(item) }}%</span>
                            </div>
                        </td>
                        <td>
                            <span v-if="item.metadata?.status" class="status-tag" :class="item.metadata.status">
                                {{ item.metadata.status }}
                            </span>
                            <span v-else-if="item.status?.inquiryStatus" class="status-tag" :class="item.status.inquiryStatus">
...
const createCharts = (): void => {
    ...
    const labels = props.rankedItems.map(item => item.title.length > 20 ? `${item.title.substring(0, 20)}...` : item.title)
    const votes = props.rankedItems.map(item => props.getItemVoteCount(item.id))
