...
const registrationInvitationText = computed(() => {
  if (sessionStore.share?.publicInquiryEmail === 'mandatory') {
    return t('agora', 'To participate, register with your email address and a name.')
  }
  if (sessionStore.share?.publicInquiryEmail === 'optional') {
    return t('agora', 'To participate, register a name and optionally with your email address.')
  }
