...
// ============================================================
// VISIBILITY - Using Inquiry Store with Relation Support
// ============================================================

const visibilityType = computed({
  get: () => inquiryStore.configuration?.visibility || 'private',
  set: (value: string) => {
    if (inquiryStore.configuration) {
      inquiryStore.configuration.visibility = value
      if (value !== 'groups') {
        inquiryStore.configuration.visibilityGroups = []
      }
      inquiryStore.write()
    }
  }
...
const visibilityGroups = computed({
  get: (): string[] => {
    return inquiryStore.configuration?.visibilityGroups || []
  },
  set: (groups: string[] | any[]): void => {
    const normalized = extractIds(groups)
    if (inquiryStore.configuration) {
      inquiryStore.configuration.visibilityGroups = normalized
      inquiryStore.write()
    }
  }
})
...
const visibilityGroupsForDisplay = computed({
  get: () => {
    const groups = inquiryStore.configuration?.visibilityGroups || []
...
// Local state that will be saved only when running lottery
...
// Computed that syncs with store when loading, but allows local edits
const participationType = computed({
  ...
  set: (value: string) => {
    ...
    if (value !== 'lottery') {
      // If switching away from lottery, save immediately
      saveParticipationPolicy(value, localParticipationConfig.value)
    } else {
      // If switching TO lottery, load the status
      loadLotteryStatus()
    }
  }
})
...
const participationConfig = computed({
  ...
  set: (config: ParticipationConfig) => {
    ...
    // Only save if not lottery mode (lottery saves on run)
...
const participationOptionLabels: Record<string, string> = {
  ...
  lottery: t('agora', 'Random Selection (Lottery)'),
}
...
const loadAccessData = async () => {
  ...
  try {
    ...
    // Only load lottery status if it's a lottery
    if (isLottery) {
...
const loadLotteryStatus = async () => {
  ...
  try {
    ...
    // Initialize with default status so UI shows
    lotteryStatus.value = {
      status: 'not_run',
      is_validated: false
    }
...
const handleRunLottery = async () => {
  ...
  try {
    ...
    // IMPORTANT: Save the lottery configuration before running
    await saveParticipationPolicy('lottery', localParticipationConfig.value)
    
    // Now get the eligible pool with the saved configuration
    poolInfo.value = await participationStore.getEligiblePool(targetType.value, targetId.value)
...
const handleLotteryConfigChange = async (field: keyof ParticipationConfig, value: any) => {
  ...
  // Don't save - will be saved when running lottery
}
    ...
    <div class="participation-section">
      ...
      <div v-if="showLotteryConfig" class="config-panel">
        <div class="config-group">
          ...
          <NcTextField
            :value="participationConfig.selection_size"
            type="number"
            min="1"
            max="100"
            :label="t('agora', 'Number of participants to select')"
            @update:model-value="(val: number) => handleLotteryConfigChange('selection_size', val)"
          />
        </div>
        ...
        <!-- Lottery Status & Actions - Always visible -->
        <div class="lottery-status">
          ...
          <div v-else-if="lotteryStatus" class="status-display">
            ...
            <div class="lottery-actions">
              <!-- No run yet -->
              <button 
                v-if="lotteryStatus.status === 'not_run'"
                class="action-button run"
              ...
              <!-- Run failed - can retry -->
              <div v-if="lotteryStatus.status === 'failed'" class="failed-actions">
                <button 
                  class="action-button run"
                  @click="handleRunLottery"
                >
                  {{ t('agora', 'Retry Lottery') }}
                </button>
                <button 
              ...
              <div v-if="lotteryStatus.status === 'completed' && !lotteryStatus.is_validated" class="completed-actions">
                <button 
          ...
          <div v-if="showLotteryResults && lotteryStatus?.run" class="results-display">
            ...
            <div class="results-grid">
              <div v-for="selection in lotteryStatus.run.selections" :key="selection.id" class="result-item">
                ...
                <span class="user">{{ selection.selected_user_id }}</span>
