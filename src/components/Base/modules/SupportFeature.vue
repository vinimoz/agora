                ...
                <div v-if="supportFeature === 'ternary' && ternaryResult" class="ternary-breakdown">
                    <div class="breakdown-item positive">
                        ...
                        <div class="breakdown-item neutral">
                            ...
                            <!-- Approval Results -->
                            <div v-if="supportFeature === 'approval_delib' && approvalDelibResult" class="binary-breakdown">
                                <div class="breakdown-item positive">
                                    <div class="breakdown-header">
                                        <ThumbIcon :supported="true" :size="iconSize * 0.8" />
                                    ...
                                    <div class="breakdown-stats">
                                        ...
                                        <span class="percentage" v-if="approvalDelibResult.totals.total > 0">({{ Math.round(approvalDelibResult.percentages.approved) }}%)</span>
...
const getGradeLabel = (grade: number): string => {
  // First try to get from the current result's grades
  if (majorityResult.value && (majorityResult.value as any).gradeLabels) {
    const labels = (majorityResult.value as any).gradeLabels
    if (labels[grade]) return labels[grade]
  }

  // Then try support_template
  if (supportTemplate.value?.grades && supportTemplate.value.grades[grade]) {
    return supportTemplate.value.grades[grade]
  }

  // Fallback to default grades for majority judgment
  const defaultGrades = [
    t('agora', 'Reject'),
  ...
  const otherGrades: Record<number, string> = {
    0: t('agora', 'Poor'),
    1: t('agora', 'Fair'),
    2: t('agora', 'Good'),
    3: t('agora', 'Very Good'),
  ...
  return defaultGrades[grade] || otherGrades[grade] || String(grade)
}
