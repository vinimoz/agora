...
const dateExpiryRelative = computed(() =>
  moment.unix(inquiryStore.configuration.expire).fromNow(),
);
...
<template>
  <div class="inquiry-information">
    ...
    <BadgeDiv>
      <template #icon>
        <PrivateInquiryIcon
          v-if="inquiryStore.configuration.access === 'private'"
        />
    ...
    <BadgeDiv>
      <template #icon>
        <HideResultsIcon
          v-if="inquiryStore.configuration.showResults === 'never'"
        />
        <ShowResultsOnClosedIcon
          v-else-if="
            inquiryStore.configuration.showResults === 'closed'
    ...
    <BadgeDiv
      v-if="
        inquiryStore.status.countParticipants
          && inquiryStore.permissions.seeResults
      "
    >
      <template #icon>
        <ParticipantsIcon />
    ...
    <BadgeDiv
      v-if="
        $route.name === 'publicInquiry' && sessionStore.currentUser.emailAddress
      "
    >
      <template #icon>
        <EmailIcon />
