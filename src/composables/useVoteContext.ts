...
export function useVoteContext(inquiryId: number): VoteContext {
  ...
  const currentUserVotes = computed(() => {
    ...
    if (!userId) return []
    return supportsStore.getSupportsByInquiryId(inquiryId).filter((s) => s.userId === userId)
  })
  ...
  const isSelectedForVote = (optionId: number): boolean => {
    ...
    if (engineId === 'quadratic')
      return (
        quadraticVotes.value[optionId] !== undefined &&
        quadraticVotes.value[optionId] !== null &&
        quadraticVotes.value[optionId] > 0
      )
    if (engineId === 'token_weighted')
      return (
        tokenWeights.value[optionId] !== undefined &&
        tokenWeights.value[optionId] !== null &&
        tokenWeights.value[optionId] > 0
      )
    if (engineId === 'approval') return selectedOptions.value.has(optionId)
    return false
  }
