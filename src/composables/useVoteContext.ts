...
export function useVoteContext(inquiryId: number): VoteContext {
    ...
    const currentUserVotes = computed(() => {
        ...
        if (!engineId) return []
            const votes = supportsStore.getSupportsByInquiryId(inquiryId).filter(
                (s) => s.userId === userId
            )
            // First try exact match
            const exact = votes.filter(s => s.supportEngineId === engineId)
            if (exact.length > 0) return exact
                // Fallback to null engine id (legacy votes)
                return votes.filter(s => s.supportEngineId === null)
    }) 
...
const canSubmitMultiVote = computed(() => {
    ...
    switch (engineId) {
        case 'binary':
            case 'ternary': {
            const scoredEntries = Object.entries(scores.value).filter(
        ...
        case 'ranking':
            ...
            const rankedEntries = Object.entries(rankings.value).filter(
                ([_, rank]) => rank !== null && rank !== undefined
            )
        ...
        case 'majority_judgment': {
            const gradedCount = Object.values(grades.value).filter(
        ...
        case 'reaction': {
            const reactedCount = Object.values(reactions.value).filter(
                (v) => v !== null && v !== undefined
            ).length
        ...
        case 'token_weighted': {
            const weights = Object.values(tokenWeights.value).filter(
                (v) => v !== null && v !== undefined && v > 0
            )
                ...
                try {
                    switch (engineId) {
                        ...
                        case 'score':
                            ...
                            const realOption = votableOptions.value.find(opt => opt.id === parseInt(optionId))
                        ...
                        case 'majority_judgment': {
                            // Build a single object { optionId: grade }
                            const gradesObj: Record<number, string> = {}
                            for (const [optId, grade] of Object.entries(grades.value)) {
                                if (grade !== null && grade !== undefined) {
                                    gradesObj[parseInt(optId)] = grade
                                }
                            }
                            if (Object.keys(gradesObj).length === 0) return false
                                await supportsStore.toggleSupport(
                                    inquiryId,
                                    0,                     // optionId = 0 (engine‑wide)
                                    userId,
                                    {} as Option,
                                    'option',
                                    { grades: gradesObj }  // send as object
                                )
                                break
                        }

                        case 'reaction':
                            for (const [optionId, reactionArr] of Object.entries(reactions.value)) {
                            if (reactionArr && reactionArr.length > 0) {
                                const realOption = votableOptions.value.find(opt => opt.id === parseInt(optionId))
                                if (!realOption) continue
                                    await supportsStore.toggleSupport(
                                        inquiryId,
                                        ...
                                        realOption,
                                        'option',
                                        reactionArr   // send array of reactions
                                    )
                            }
                        }
                        break
                        ...
                        case 'ranking':
                            case 'condorcet':
                            case 'borda': {
                            // Build ranking object { optionId: rank }
                            const rankingObj: Record<number, number> = {}
                            for (const [optId, rank] of Object.entries(rankings.value)) {
                                if (rank !== null && rank !== undefined) {
                                    rankingObj[parseInt(optId)] = rank
                                }
                            }
                            if (Object.keys(rankingObj).length === 0) return false
                                await supportsStore.toggleSupport(inquiryId, 0, userId, {} as Option, 'option', {
                                    ranking: rankingObj,
                                })
                                break
                        }
                        ...
                        case 'quadratic': {
                            ...
                            for (const [optId, votes] of Object.entries(quadraticVotes.value)) {
                                if (votes !== null && votes !== undefined && votes > 0) {
                                    scoresObj[parseInt(optId)] = votes
                                }
                            }
                        ...
                        case 'token_weighted': {
                            ...
                            for (const [optId, weight] of Object.entries(tokenWeights.value)) {
                                if (weight !== null && weight !== undefined && weight > 0) {
                                    weightsObj[parseInt(optId)] = weight
                                }
                            }
                            if (Object.keys(weightsObj).length === 0) return false
                                await supportsStore.toggleSupport(inquiryId, 0, userId, {} as Option, 'option', {
                                    scores: weightsObj,
                                })
                                break
                        ...
                        case 'binary':
                            ...
                            for (const [optId, value] of Object.entries(scores.value)) {
                                if (value !== null && value !== undefined) {
                                    votesObj[parseInt(optId)] = value as number
                                }
                            }
                            if (Object.keys(votesObj).length === 0) return false
                                await supportsStore.toggleSupport(inquiryId, 0, userId, {} as Option, 'option', {
                                    scores: votesObj
                                })
                                break
                        ...
                        case 'phased_voting': {
                            const currentRound = (currentEngine.value?.config?.current_round as number) || 1;
                            const selected = Array.from(selectedOptions.value);
                            if (selected.length === 0) return false;
                            await supportsStore.toggleSupport(
                                inquiryId, 0, userId, {} as Option, 'option',
...
const loadResults = async () => {
    ...
    if (!engineId) return
        ...
        try {
            let engineResults = resultStore.getResultsByEngine(engineId)

            if (engineResults.length === 0 && inquiryStore.status?.supportResult) {
                // First try exact match
                let inquiryResults = inquiryStore.status.supportResult.filter(
                    (r: SupportResult) => r.support_engine_id === engineId
                )
                // Fallback to results with null engine_id
                if (inquiryResults.length === 0) {
                    inquiryResults = inquiryStore.status.supportResult.filter(
                        (r: SupportResult) => r.support_engine_id === null
                    )
                }
                if (inquiryResults.length) {
                    resultStore.setResults(inquiryResults)
            ...
            if (engineResults.length === 0) {
                try {
                    ...
                    if (error?.name === 'CanceledError' || error?.message?.includes('canceled')) {
                        console.log('Result calculation was cancelled (expected)')
                        return
                    }
                    throw error
                }
    ...
    if (!result) return 0
        ...
        switch (result.type) {
            ...
            case 'star':
                return result.totals?.total || 0
            case 'majority_judgment':
                // For majority judgment, return total votes for this option
            ...
            case 'ranking':
                // Return average rank (lower is better). For sorting, we'll handle separately.
                return result.rankings?.[optionId] || 0
            case 'condorcet':
                // Return wins count (or losses) – depends on what you want to sort by
                return result.wins?.[optionId] || 0
            case 'borda':
                return result.scores?.[optionId] || 0
            case 'quadratic':
                return result.scores?.[optionId] || 0
            case 'token_weighted':
                return result.weights?.[optionId] || 0
            case 'phased_voting':
                return result.counts?.[optionId] || 0
            default:
                return 0
...
const totalVotes = computed(() => {
    ...
    for (const result of resultsMap.value.values()) {
        switch (result.type) {
            ...
            break
            case 'ranking':
                total += result.total_voters || Object.keys(result.rankings || {}).length
            break
            case 'condorcet':
            ...
            break
            case 'token_weighted':
                total += result.participant_count || 0
            break
            case 'phased_voting':
                total += Object.values(result.counts || {}).reduce((a, b) => a + b, 0)
            break
...
function loadUserVotesForEngine(engineId: number) {
    ...
    if (!userId) return
        ...
        // Get all supports for this user, engine, and inquiry
        const userVotes = supportsStore.getSupportsByInquiryId(inquiryId)
        ...
        for (const vote of userVotes) {
            ...
            // 1. Approval (multi‑select) – value = { selected: [4,10] }
            if (value && typeof value === 'object' && Array.isArray(value.selected)) {
                value.selected.forEach((id: number) => selectedOptions.value.add(id))
                continue
            }

            // 2. Ranking / Condorcet / Borda – value = { ranking: { 4:1, 10:2 } }
            if (value && typeof value === 'object' && value.ranking) {
            ...
            // 3. Quadratic – value = { scores: { 4:1, 10:3 } }
            if (value && typeof value === 'object' && value.scores && effectiveEngineId.value === 'quadratic') {
            ...
            if (optionId === 0 && value && typeof value === 'object' && value.scores) {
                for (const [optId, val] of Object.entries(value.scores)) {
                    scores.value[Number(optId)] = val as number
                }
                continue
            }
            // 4. Token Weighted – value = { scores: { 4:100 } }
            if (value && typeof value === 'object' && value.scores && effectiveEngineId.value === 'token_weighted') {
            ...
            if (optionId === 0 && value && typeof value === 'object' && value.grades) {
            ...
            if (optionId === 0 && value && typeof value === 'object' && value.scores) {
                if (effectiveEngineId.value === 'quadratic') {
                    for (const [optId, votes] of Object.entries(value.scores)) {
                        quadraticVotes.value[Number(optId)] = votes as number;
                    }
                } else if (effectiveEngineId.value === 'token_weighted') {
                    for (const [optId, weight] of Object.entries(value.scores)) {
                        tokenWeights.value[Number(optId)] = weight as number;
                    }
                }
            ...
            // 5. Per‑option votes (binary, ternary, score, star, majority judgment, reaction)
            if (optionId > 0) {
                const engineId = effectiveEngineId.value

                // Binary / Ternary / Score / Star – value is a number
                if (typeof value === 'number') {
                    if (engineId === 'score' || engineId === 'star') {
                        scores.value[optionId] = value
                    } else {
                        // Binary & ternary are handled by `hasUserVotedFor` via currentUserVotes,
                        // but we still need to mark them so the UI shows the current selection.
                        // For these simple engines we only need to know that the user voted,
                        // no extra local state required because VoteInput components use userVote prop.
                    }
                    continue
                }

                // Majority Judgment – value is a string (grade)
                if (typeof value === 'string' && engineId === 'majority_judgment') {
                    grades.value[optionId] = value
                    continue
                }

                // Reaction – value is an array of strings (emojis)
                if (Array.isArray(value) && engineId === 'reaction') {
...
const getRankedOptions = (options: Option[]): Option[] => {
    ...
    if (engineId === 'ranking') {
        return [...options].sort((a, b) => 
                                 (getOptionVoteCount(a.id) || Infinity) - (getOptionVoteCount(b.id) || Infinity)
                                )
    }
    if (engineId === 'condorcet') {
        return [...options].sort((a, b) => 
                                 (getOptionVoteCount(b.id) || 0) - (getOptionVoteCount(a.id) || 0)
                                )
    }
    if (engineId === 'borda') {
        return [...options].sort((a, b) => 
...
watch(() => supportsStore.supports, (supports) => {
    console.log('🔍 All supports for inquiry', inquiryId, supports)
}, { deep: true, immediate: true })

watch(currentUserVotes, (votes) => {
    console.log('👤 Current user votes (filtered):', votes)
}, { immediate: true })
