...
export function useVoteContext(inquiryId: number): VoteContext {
  ...
  const getUserVoteValueForOption = (optionId: number): SupportValue | null => {
    ...
    if (engineVote && typeof engineVote.value === 'object') {
      ...
      if (val.selected && Array.isArray(val.selected)) return val.selected.includes(optionId) ? 1 : null
    }
    return null
  }
  ...
  const canSubmitMultiVote = computed(() => {
    ...
    switch (engineId) {
      case 'binary':
      case 'ternary':
        return Object.values(scores.value).filter((v) => v !== null && v !== undefined).length > 0
      case 'approval': {
      ...
      case 'condorcet':
      case 'borda': {
        const ranked = Object.entries(rankings.value).filter(([_, r]) => r !== null && r !== undefined)
        if (ranked.length < 2) return false
        const ranks = new Set<number>()
        for (const [_, r] of ranked) {
          if (ranks.has(r as number)) return false
          ranks.add(r as number)
        }
        for (const [_, r] of ranked) {
          if ((r as number) < 1 || (r as number) > maxRank.value) return false
        }
        return true
      }
      case 'score':
      case 'star': {
        ...
        for (const [_, s] of scored) {
          if ((s as number) < scoreMin.value || (s as number) > scoreMax.value) return false
        }
        return true
      }
      case 'majority_judgment':
        return Object.values(grades.value).filter((g) => g !== null && g !== undefined).length > 0
      case 'reaction':
        return Object.values(reactions.value).filter((r) => r !== null && r !== undefined).length > 0
      case 'quadratic': {
        const votes = Object.values(quadraticVotes.value).filter((v) => v !== null && v !== undefined && v > 0)
      ...
      case 'token_weighted':
        return Object.values(tokenWeights.value).filter((w) => w !== null && w !== undefined && w > 0).length > 0
      case 'phased_voting':
    ...
    switch (engineType) {
      ...
      case 'token_weighted': {
        ...
        for (const [optId, weight] of Object.entries(tokenWeights.value)) {
          if (weight !== null && weight !== undefined && weight > 0) scoresObj[Number(optId)] = weight
        }
        if (Object.keys(scoresObj).length === 0) return false
        payload = { scores: scoresObj }
        break
  ...
  const getOptionVoteCount = (optionId: number): number => {
    ...
    if (perOptionEngines.includes(engineId)) {
      ...
      if (result) {
        switch (result.type) {
          case 'binary': return result.options?.[optionId]?.yes ?? result.totals?.yes ?? 0
          case 'ternary': return result.options?.[optionId]?.yes ?? result.totals?.yes ?? 0
          case 'score':
          case 'star': return result.options?.[optionId]?.total ?? result.totals?.total ?? 0
          case 'reaction': {
          ...
          case 'majority_judgment': return result.options?.[optionId]?.total_votes ?? 0
        }
      }
      ...
      if (res) {
        switch (res.type) {
          case 'binary': return res.options?.[optionId]?.yes ?? res.totals?.yes ?? 0
          case 'ternary': return res.options?.[optionId]?.yes ?? res.totals?.yes ?? 0
          case 'score':
          case 'star': return res.options?.[optionId]?.total ?? res.totals?.total ?? 0
          case 'reaction': {
            const perOpt = res.options?.[optionId]
            if (perOpt) return Object.values(perOpt).reduce((a, b) => a + b, 0)
            return Object.values(res.counts || {}).reduce((a, b) => a + b, 0)
          }
          case 'majority_judgment': return res.options?.[optionId]?.total_votes ?? 0
        }
      }
      return 0
    }
    ...
    switch (engineId) {
      case 'approval': return res.counts?.[optionId] || 0
      case 'ranking': return res.rankings?.[optionId] || 0
      case 'condorcet': return res.wins?.[optionId] || 0
      case 'borda': return res.scores?.[optionId] || 0
      case 'quadratic': return res.scores?.[optionId] || 0
      case 'token_weighted': return res.weights?.[optionId] || 0
      case 'phased_voting': return res.counts?.[optionId] || 0
      case 'approval_delib': return res.totals?.approved || 0
      default: return 0
    }
  }
  ...
  const totalVotes = computed(() => {
    ...
    if (res && perOptionEngines.includes(engineId)) {
      ...
      if (res.options) {
        ...
        if (res.type === 'ternary') return (res.totals.yes || 0) + (res.totals.no || 0) + (res.totals.abstain || 0)
        if (res.type === 'score' || res.type === 'star') return res.totals.total || 0
        if (res.type === 'reaction') return Object.values(res.counts || {}).reduce((a, b) => a + b, 0)
    ...
    if (perOptionEngines.includes(engineId)) {
      ...
      for (const result of resultsMap.value.values()) {
        switch (result.type) {
          case 'binary': total += (result.totals?.yes || 0) + (result.totals?.no || 0); break
          case 'ternary': total += (result.totals?.yes || 0) + (result.totals?.no || 0) + (result.totals?.abstain || 0); break
          case 'score':
          case 'star': total += result.totals?.total || 0; break
          case 'majority_judgment': total += result.total_votes || 0; break
          case 'reaction': total += Object.values(result.counts || {}).reduce((a, b) => a + b, 0); break
        }
      }
      return total
    }
    ...
    if (!res) return 0
    switch (engineId) {
      case 'approval': return Object.values(res.counts || {}).reduce((a, b) => a + b, 0)
      case 'ranking': return res.total_voters || Object.keys(res.rankings || {}).length
      case 'condorcet':
      case 'borda': return res.total_voters || 0
      case 'quadratic': return res.total_votes || 0
      case 'token_weighted': return res.participant_count || 0
      case 'phased_voting': return Object.values(res.counts || {}).reduce((a, b) => a + b, 0)
      default: return 0
    }
  })
  ...
  const getRankedOptions = (options: Option[]): Option[] => {
    const engineId = effectiveEngineId.value
    if (engineId === 'ranking') {
      return [...options].sort((a, b) => (getOptionVoteCount(a.id) || Infinity) - (getOptionVoteCount(b.id) || Infinity))
    }
    if (['condorcet', 'borda'].includes(engineId)) {
      return [...options].sort((a, b) => (getOptionVoteCount(b.id) || 0) - (getOptionVoteCount(a.id) || 0))
    }
  ...
  const loadResults = () => {
    ...
    if (inquiry.status?.supportResult) {
      ...
      const engineResultEntry = results.find(r => r.support_engine_id === selectedEngineId.value && r.target_type === 'inquiry')
  ...
  const scoreMin = computed(() => {
    ...
    return effectiveEngineId.value === 'star' ? (min ?? 1) : (min ?? 0)
  })
