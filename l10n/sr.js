OC.L10N.register(
    "agora",
    {
    "Inquiry changes" : "Упитник се мења",
    "Agora" : "Агора",
    "Team" : "Тим",
    "Contact group" : "Контакт група",
    "Group" : "Група",
    "Inquiry \"%s\" - Confirmation" : "Упитник „%s” - Потврда",
    "This email is sent to you to inform you about the result of a inquiry you participated in. At least your name or your email address was recorded in this inquiry. If you want to be removed from this inquiry, contact the site administrator or the inquiry initiator, where the mail is sent from." : "Ова порука Вам је послата као обавештење о резултатима упитника у којем сте учествовали. У овом упитнику су забележени бар Ваше име или адреса е-поште. Ако желите да будете уклоњени из упитника, обратите се покретачу упитника или администратору сајта, са кога је ова порука и послата.",
    "{owner} wants to inform you about the final result of the inquiry \"{title}\"" : "{owner} жели да вас обавести о коначним резултатима упитника „{title}”",
    "_Confirmed option:_::_Confirmed options:_" : ["Потврђена опција:","Потврђене опције:","Потврђене опције:"],
    "The used time zone is \"%s\", based on the detected time zone at your registration time. To view the times in your current time zone, enter the inquiry by clicking the button below." : "Користи се временска зона „%s”, према откривеној временској зони у време ваше регистрације. Ако желите да видите времена у својој тренутној временској зони, уђите на упитник кликом на доње дугме.",
    "This link gives you personal access to the inquiry named above. Press the button above or copy the following link and add it in your browser's location bar:" : "Ова веза Вам даје лични приступ упитнику наведеном изнад. Притисните дугме изнад или копирајте следећу везу и додајте је на адресну траку вашег веб прегледача:",
    "Do not share this link with other people, because it is connected to your supports." : "Овај линк немојте да делите са другим људима јер је везан за ваше учешће.",
    "Inquiry invitation \"%s\"" : "Позив на упитник „%s“",
    "This email is sent to you, because you are invited to support in this inquiry by the inquiry owner. At least your name or your email address is recorded in this inquiry. If you want to get removed from this inquiry, contact the site administrator or the initiator of this inquiry, where the mail is sent from." : "Ова порука Вам је послата зато што Вас је власник упитника позвао да учествујете у упитнику. У овом упитнику су уписани бар Ваше име или адреса е-поште. Ако желите да будете уклоњени из упитника, обратите се покретача упитника или администратору сајта, са кога је ова порука и послата.",
    "{owner} invited you to take part in the inquiry \"{title}\" as a member of the group {group_name}" : "{owner} вас је позвао да учествујете у упитнику „{title}” као члан групе {group_name}",
    "{owner} invited you to take part in the inquiry \"{title}\"" : "{owner} Вас је позвао да учествујете у упитнику „{title}“",
    "Notification for inquiry \"%s\"" : "Обавештење за упитник „%s”",
    "Go to inquiry" : "Иди на упитник",
    "This email is sent to you, because you subscribed to notifications of this inquiry. To opt out, visit the inquiry and remove your subscription." : "Овај и-мејл Вам је послат пошто сте претплаћени на обавештења о овом упитнику. Ако желите да се одјавите, посетите упитник и уклоните Вашу претплату.",
    "Legal Notice" : "Правно обавештење",
    "Privacy Policy" : "Политика приватности",
    "Agora App - New Activity" : "Апликација Агора - Нова активност",
    "\"{title}\" has recent activity:" : "„{title}” има скорашњу активност: ",
    "A participant" : "Учесник",
    "%s has supported." : "%s је подржао.",
    "Updated inquiry configuration. Please check your supports." : "Конфигурација упитника је ажурирана. Молимо вас да проверите своја подржавања.",
    "The inquiry has been deleted." : "Упитник је обрисан.",
    "The inquiry has been restored." : "Упитник је враћен.",
    "The inquiry has been closed." : "Упитник је затворен.",
    "A voting option has been added." : "Додата су опција гласања.",
    "A voting option has been changed." : "Измењена је опција гласања.",
    "A voting option has been confirmed." : "Потврђена је опција гласања.",
    "A voting option has been removed." : "Уклоњена је опција гласања.",
    "The inquiry owner has been changed." : "Промењен је власник упитика.",
    "%s created the inquiry." : "%s је креирао упитник.",
    "The inquiry has been reopened." : "Упитник је поново отворен.",
    "A voting option has been unconfirmed." : "Опција гласања више није потврђена.",
    "%s has left a comment." : "%s је оставио коментар.",
    "%s has support." : "%s подржава.",
    "Reminder for inquiry \"%s\"" : "Подсетник за упитник „%s”",
    "Check your supports" : "Проверите своја подржавања",
    "The first inquiry option is away less than {leftPeriod} hours ({dateTime}, {timezone})." : "Прва опција упитника удаљена мање од {leftPeriod} сати ({dateTime}, {timezone}).",
    "The inquiry is about to expire in less than {leftPeriod} hours ({dateTime}, {timezone})." : "Упитник истиче за мање од {leftPeriod} сати ({dateTime}, {timezone}).",
    "{owner} sends you this reminder to make sure, your supports are set." : "{owner} вам шаље овај подсетник да провери да ли су постављена ваша подржавања.",
    "Contact" : "Контакт",
    "External Email" : "Спољни и-мејл",
    "External participant" : "Спољни учесник",
    "User" : "Корисник",
    "%s invited you to a inquiry" : "%s вас је позвао у упитник",
    "{actor} has invited you to the inquiry \"%s\"." : "{actor} вас је позвао на упитник „%s”.",
    "%s took over your inquiry" : "%s је преузео ваш упитник",
    "{actor} took over your inquiry \"%s\" and is the new owner." : "{actor} је преузео ваш упитник „%s” и сада је он нови власник.",
    "%s is the new owner of your inquiry." : "%s је нови власник вашег упитника.",
    "{actor} transfered your inquiry \"%s\" to {newOwner}. You are no more the owner." : "{actor} је пренео ваш упитник „%s” на {newOwner}. Више нисте власник.",
    "%s deleted your inquiry" : "%s је обрисао ваш упитник",
    "{actor} deleted your inquiry \"%s\"." : "{actor} је обрисао ваш упитник „%s”.",
    "%s archived your inquiry" : "%s је архивирао ваш упитник",
    "{actor} archived your inquiry \"%s\"." : "{actor} је архивирао ваш упитник „%s”.",
    "Inquiry: %s" : "Упитник: %s",
    "404 - Inquiry not found" : "404 - Упитник није пронађен",
    "This inquiry does not exist (anymore)." : "Овај упитник (више) не постоји.",
    "No one." : "Нико.",
    "Access denied" : "Забрањен приступ",
    "You have no access to this inquiry. Contact %s if you think this is a mistake." : "Немате приступ овом упитнику. Контактирајте %s ако мислите да је ово грешка.",
    "No description available." : "Нема доступног описа.",
    "Inquiries" : "Упитници",
    "You have supported this inquiry {inquiryTitle}" : "Подржали сте овај упитник {inquiryTitle}",
    "{actor} has supported this inquiry {inquiryTitle}" : "{actor} је подржао овај упитник {inquiryTitle}",
    "You have supported" : "Подржали сте",
    "{actor} has supported" : "{actor} је подржао",
    "You have deleted a support from inquiry {inquiryTitle}" : "Обрисали сте подршку из упитника {inquiryTitle}",
    "{actor} has deleted a support from inquiry {inquiryTitle}" : "{actor} је обрисао подршку из упитника {inquiryTitle}",
    "You have deleted a support" : "Обрисали сте подршку",
    "{actor} has deleted a support" : "{actor} је обрисао подршку",
    "You have commented on inquiry {inquiryTitle}" : "Поставили сте коментар на упитник {inquiryTitle}",
    "{actor} has commented on inquiry {inquiryTitle}" : "{actor}је поставио коментар на упитник {inquiryTitle}",
    "You have commented" : "Постави сте коментар",
    "{actor} has commented" : "{actor} је поставио коментар",
    "You have deleted a comment from inquiry {inquiryTitle}" : "Обрисали сте коментар из упитника {inquiryTitle}",
    "{actor} has deleted a comment from inquiry {inquiryTitle}" : "{actor} је обрисао коментар из упитника {inquiryTitle}",
    "You have deleted a comment" : "Обрисали сте коментар",
    "{actor} has deleted a comment" : "{actor} је обрисао коментар",
    "You have restored a comment from inquiry {inquiryTitle}" : "Вратили сте коментар у упитник {inquiryTitle}",
    "{actor} has restored a comment from inquiry {inquiryTitle}" : "{actor} је вратио коментар у упитник {inquiryTitle}",
    "You have restored a comment" : "Вратили сте коментар",
    "{actor} has restored a comment" : "{actor} је вратио коментар",
    "You have added an option to inquiry {inquiryTitle}" : "Додали сте опцију у упитник {inquiryTitle}",
    "{actor} has added an option to inquiry {inquiryTitle}" : "{actor} је додао опцију упитник {inquiryTitle}",
    "You have added an option" : "Додали сте опцију",
    "{actor} has added an option" : "{actor} је додао опцију",
    "You have changed an option of inquiry {inquiryTitle}" : "Изменили сте опцију упитника {inquiryTitle}",
    "{actor} has changed an option of inquiry {inquiryTitle}" : "{actor} је изменио опцију упитника {inquiryTitle}",
    "You have changed an option" : "Изменили сте опцију гласања",
    "{actor} has changed an option" : "{actor} је изменио опцију",
    "You have confirmed option {optionTitle} of inquiry {inquiryTitle}" : "Потврдили сте опцију {optionTitle} упитника {inquiryTitle}",
    "{actor} has confirmed option {optionTitle} of inquiry {inquiryTitle}" : "{actor} је потврдио опцију {optionTitle} упитника {inquiryTitle}",
    "You have confirmed option {optionTitle}" : "Потврдили сте опцију {optionTitle}",
    "{actor} has confirmed option {optionTitle}" : "{actor} је потврдио опцију {optionTitle}",
    "You have unconfirmed option {optionTitle} of inquiry {inquiryTitle}" : "Имате непотвђену опцију {optionTitle} у упитнику {inquiryTitle}",
    "{actor} has unconfirmed option {optionTitle} of inquiry {inquiryTitle}" : "{actor} је уклонио потврду опције {optionTitle} упитника {inquiryTitle}",
    "You have unconfirmed option {optionTitle}" : "Уклонили сте потврду опције {optionTitle}",
    "{actor} has unconfirmed option {optionTitle}" : "{actor} је уклонио потврду опције {optionTitle}",
    "You have deleted option {optionTitle} from inquiry {inquiryTitle}" : "Обрисали сте опцију {optionTitle} из упитника {inquiryTitle}",
    "{actor} has deleted option {optionTitle} from inquiry {inquiryTitle}" : "{actor} је обрисао опцију {optionTitle} из упитника {inquiryTitle}",
    "You have deleted option {optionTitle}" : "Обрисали сте опцију {optionTitle}",
    "{actor} has deleted option {optionTitle}" : "{actor} је обрисао опцију {optionTitle}",
    "You have restored option {optionTitle} from inquiry {inquiryTitle}" : "Вратили сте опцију {optionTitle} из упитника {inquiryTitle}",
    "{actor} has restored option {optionTitle} from inquiry {inquiryTitle}" : "{actor} је вратио опцију {optionTitle} из упитника {inquiryTitle}",
    "You have restored option {optionTitle}" : "Вратили сте опцију {optionTitle}",
    "{actor} has restored option {optionTitle}" : "{actor} је вратио опцију {optionTitle}",
    "You have added inquiry {inquiryTitle}" : "Додали сте упитник {inquiryTitle}",
    "{actor} has added inquiry {inquiryTitle}" : "{actor} је додао упитник {inquiryTitle}",
    "You have created this inquiry" : "Ви сте креирали овај упитник",
    "{actor} has created this inquiry" : "{actor} је креирао овај упитник",
    "You have changed the configuration of inquiry {inquiryTitle}" : "Изменили сте конфигурацију упитника {inquiryTitle}",
    "{actor} has changed the configuration of inquiry {inquiryTitle}" : "{actor} је изменио конфигурацију упитника {inquiryTitle}",
    "You have changed the configuration" : "Изменили сте конфигурацију",
    "{actor} has changed the configuration" : "{actor} је изменио конфигурацију",
    "You have archived inquiry {inquiryTitle}" : "Архивирали сте упитник {inquiryTitle}",
    "{actor} has archived inquiry {inquiryTitle}" : "{actor} је архивирао упитник {inquiryTitle}",
    "You have archived this inquiry" : "Ви сте архивирали овај упитник",
    "{actor} has archived this inquiry" : "{actor} је архивирао овај упитник",
    "You have restored inquiry {inquiryTitle}" : "Вратили сте упитник {inquiryTitle}",
    "{actor} has restored inquiry {inquiryTitle}" : "{actor} је вратио упитник {inquiryTitle}",
    "You have restored this inquiry" : "Ви сте вратили овај упитник",
    "{actor} has restored this inquiry" : "{actor} је вратио овај упитник",
    "Inquiry {inquiryTitle} has been closed" : "Упитник {inquiryTitle} је затворен",
    "This inquiry has been closed" : "Овај упитник је затворен",
    "You have closed the inquiry \"{inquiryTitle}\"" : "Затворили сте упитник „{inquiryTitle}”",
    "{actor} has closed the inquiry \"{inquiryTitle}\"" : "{actor} је затворио упитник „{inquiryTitle}”",
    "You have closed this inquiry" : "Ви сте затворили овај упитник",
    "{actor} has closed this inquiry" : "{actor} је затворио овај упитник",
    "You have reopened the inquiry \"{inquiryTitle}\"" : "Поново сте отворили упитник „{inquiryTitle}”",
    "{actor} has reopened the inquiry \"{inquiryTitle}\"" : "{actor} је поново отворио упитник „{inquiryTitle}”",
    "You have reopened this inquiry" : "Ви сте поново отворили овај упитник",
    "{actor} has reopened this inquiry" : "{actor} је поново отворио овај упитник",
    "You have changed the owner of inquiry {inquiryTitle}" : "Изменили сте власника упитника {inquiryTitle}",
    "{actor} has changed the owner of inquiry {inquiryTitle}" : "{actor} је изменио власника упитника {inquiryTitle}",
    "You have changed the inquiry owner" : "Променили сте власника упитника",
    "{actor} has changed the inquiry owner" : "{actor} је променио власника упитника",
    "You have reordered the options of inquiry {inquiryTitle}" : "Изменили сте редослед опција у упитнику {inquiryTitle}",
    "{actor} has reordered the options of inquiry {inquiryTitle}" : "{actor} је изменио редослед опција у упитнику {inquiryTitle}",
    "You have reordered the options" : "Изменили сте редослед опција у гласању",
    "{actor} has reordered the options" : "{actor} је изменио редослед опција у гласању",
    "You have changed your email address" : "Променили сте своју и-мејл адресу",
    "{sharee} has changed the email address" : "{sharee} је променио и-мејл адресу",
    "Email address of {sharee} has been changed" : "И-мејл адреса {sharee} је промењена",
    "You have changed your name" : "Променили сте своје име",
    "{sharee} has changed the name" : "{sharee} је променио име",
    "Display name of {sharee} has been changed" : "Име за приказ {sharee} је промењено",
    "You have changed the share label" : "Променили сте ознаку дељења",
    "You have changed the share type" : "Променили сте тип дељења",
    "{actor} has changed the share type" : "{actor} је променио тип дељења",
    "You have changed the registration constraints for the public share labeled {sharee}" : "Изменили сте ограничења регистрације за јавно дељење означено са {sharee}",
    "{actor} has changed the registration constraints for the public share labeled {sharee}" : "{actor} је изменио ограничења регистрације за јавно дељење означено са {sharee}",
    "You have registered to inquiry {inquiryTitle}" : "Регистровали сте се за упитник {inquiryTitle}",
    "{sharee} registered to inquiry {inquiryTitle}" : "{sharee} се регистровао за упитник {inquiryTitle}",
    "You have registered" : "Регистровали сте се",
    "{sharee} has registered" : "{sharee} се регистровао",
    "You have supported in inquiry {inquiryTitle}" : "Дали сте подршку у упитнику {inquiryTitle}",
    "{actor} has supported in inquiry {inquiryTitle}" : "{actor} је дао подршку у упитнику {inquiryTitle}",
    "You have locked the share of {sharee}" : "Закључали сте дељење {sharee}",
    "{actor} has locked the share of {sharee}" : "{actor} је закључао дељење {sharee}",
    "You have unlocked the share of {sharee}" : "Откључали сте дељење {sharee}",
    "{actor} has unlocked the share of {sharee}" : "{actor} је откључао дељење {sharee}",
    "You have added a public share to inquiry {inquiryTitle}" : "Додали сте јавно дељење у упитник {inquiryTitle}",
    "{actor} has added a public share to inquiry {inquiryTitle}" : "{actor} је додао јавно дељење у упитник {inquiryTitle}",
    "You have added a public share" : "Додали сте јавно гласање",
    "{actor} has added a public share" : "{actor} је додао јавно гласање",
    "You have shared inquiry {inquiryTitle} with group {sharee}" : "Поделили сте упитник {inquiryTitle} са групом {sharee}",
    "{actor} has shared inquiry {inquiryTitle} with group {sharee}" : " {actor} је поделио упитник {inquiryTitle} са групом {sharee}",
    "You have shared this inquiry with group {sharee}" : "Поделили сте овај упитник са групом {sharee}",
    "{actor} has shared this inquiry with group {sharee}" : "{actor} је поделио овај упитник са групом {sharee}",
    "You have shared inquiry {inquiryTitle} with circle {sharee}" : "Поделили сте упитник {inquiryTitle} са кругом {sharee}",
    "{actor} has shared inquiry {inquiryTitle} with circle {sharee}" : " {actor} је поделио упитник {inquiryTitle} са кругом {sharee}",
    "You have shared this inquiry with circle {sharee}" : "Поделили сте овај упитник са кругом {sharee}",
    "{actor} has shared this inquiry with circle {sharee}" : "{actor} је поделио овај упитник са кругом {sharee}",
    "You have shared inquiry {inquiryTitle} with contact group {sharee}" : "Поделили сте упитник {inquiryTitle} са групом контаката {sharee}",
    "{actor} has shared inquiry {inquiryTitle} with contact group {sharee}" : " {actor} је поделио упитник {inquiryTitle} са групом контаката {sharee}",
    "You have shared this inquiry with contact group {sharee}" : "Поделили сте овај упитник са групом контаката {sharee}",
    "{actor} has shared this inquiry with contact group {sharee}" : "{actor} је поделио овај упитник са групом контаката {sharee}",
    "You have shared inquiry {inquiryTitle} with {sharee}" : "Поделили сте упитник {inquiryTitle} са {sharee}",
    "{actor} has shared inquiry {inquiryTitle} with {sharee}" : " {actor} је поделио упитник {inquiryTitle} са {sharee}",
    "You have shared this inquiry with {sharee}" : "Поделили сте овај упитник са {sharee}",
    "{actor} has shared this inquiry with {sharee}" : "{actor} је поделио овај упитник са {sharee}",
    "You have deleted the share for {sharee} from inquiry {inquiryTitle}" : "Обрисали сте дељење за {sharee} из упитника {inquiryTitle}",
    "{actor} has deleted the share for {sharee} from inquiry {inquiryTitle}" : "{actor} је обрисао дељење за {sharee} из упитника {inquiryTitle}",
    "You have deleted share of {sharee}" : "Обрисали сте дељење {sharee}",
    "{actor} has deleted a share" : "{actor} је обрисао дељење",
    "You have deleted a public share from inquiry {inquiryTitle}" : "Обрисали сте јавно дељење из упитника {inquiryTitle}",
    "{actor} has deleted a public share from inquiry {inquiryTitle}" : "{actor} је обрисао јавно дељење из упитника {inquiryTitle}",
    "You have deleted a public share" : "Обрисали сте јавно дељење",
    "{actor} has deleted a public share" : "{actor} је обрисао јавно дељење",
    "You have deleted the share for group {sharee} from inquiry {inquiryTitle}" : "Обрисали сте дељење за групу {sharee} из упитника {inquiryTitle}",
    "{actor} has deleted the share for group {sharee} from inquiry {inquiryTitle}" : "{actor} је обрисао дељење за групу {sharee} из упитника {inquiryTitle}",
    "You have deleted the share for group {sharee}" : "Обрисали сте дељење за групу {sharee}",
    "{actor} has deleted the share for group {sharee}" : "{actor} је обрисао дељење за групу {sharee}",
    "You have deleted the share for circle {sharee} from inquiry {inquiryTitle}" : "Обрисали сте дељење за круг {sharee} из упитника {inquiryTitle}",
    "{actor} has deleted the share for circle {sharee} from inquiry {inquiryTitle}" : "{actor} је обрисао дељење за круг {sharee} из упитника {inquiryTitle}",
    "You have deleted the share for circle {sharee}" : "Обрисали сте дељење за круг {sharee}",
    "{actor} has deleted the share for circle {sharee}" : "{actor} је обрисао дељење за круг {sharee}",
    "You have deleted the share for contact group {sharee} from inquiry {inquiryTitle}" : "Обрисали сте дељење за групу контаката {sharee} из упитника {inquiryTitle}",
    "{actor} has deleted the share for contact group {sharee} from inquiry {inquiryTitle}" : "{actor} је обрисао дељење за групу контаката {sharee} из упитника {inquiryTitle}",
    "You have deleted the share for contact group {sharee}" : "Обрисали сте дељење за групу контаката {sharee}",
    "{actor} has deleted the share for contact group {sharee}" : "{actor} је обрисао дељење за групу контаката {sharee}",
    "You have deleted a share from inquiry {inquiryTitle}" : "Обрисали сте дељење из упитника {inquiryTitle}",
    "{actor} has deleted a share from inquiry {inquiryTitle}" : "{actor} је обрисао дељење из упитника {inquiryTitle}",
    "You have restored the share for {sharee} from inquiry {inquiryTitle}" : "Вратили сте дељење за {sharee} у упитник {inquiryTitle}",
    "{actor} has restored the share for {sharee} from inquiry {inquiryTitle}" : "{actor} је вратио дељење за {sharee} у упитник {inquiryTitle}",
    "You have restored share of {sharee}" : "Вратили сте дељење {sharee}",
    "{actor} has restored a share" : "{actor} је вратио дељење",
    "You have restored a public share from inquiry {inquiryTitle}" : "Вратили сте јавно дељење у упитник {inquiryTitle}",
    "{actor} has restored a public share from inquiry {inquiryTitle}" : "{actor} је вратио јавно дељење у упитник {inquiryTitle}",
    "You have restored a public share" : "Вратили сте јавно дељење",
    "{actor} has restored a public share" : "{actor} је вратио јавно дељење",
    "You have restored the share for group {sharee} from inquiry {inquiryTitle}" : "Вратили сте дељење за групу {sharee} у упитник {inquiryTitle}",
    "{actor} has restored the share for group {sharee} from inquiry {inquiryTitle}" : "{actor} је вратио дељење за групу {sharee} у упитник {inquiryTitle}",
    "You have restored the share for group {sharee}" : "Вратили сте дељење за групу {sharee}",
    "{actor} has restored the share for group {sharee}" : "{actor} је вратио дељење за групу {sharee}",
    "You have restored the share for circle {sharee} from inquiry {inquiryTitle}" : "Вратили сте дељење за круг {sharee} у упитник {inquiryTitle}",
    "{actor} has restored the share for circle {sharee} from inquiry {inquiryTitle}" : "{actor} је вратио дељење за круг {sharee} у упитник {inquiryTitle}",
    "You have restored the share for circle {sharee}" : "Вратили сте дељење за круг {sharee}",
    "{actor} has restored the share for circle {sharee}" : "{actor} је вратио дељење за круг {sharee}",
    "You have restored the share for contact group {sharee} from inquiry {inquiryTitle}" : "Вратили сте дељење за групу контаката {sharee} у упитник {inquiryTitle}",
    "{actor} has restored the share for contact group {sharee} from inquiry {inquiryTitle}" : "{actor} је вратио дељење за групу контаката {sharee} у упитник {inquiryTitle}",
    "You have restored the share for contact group {sharee}" : "Вратили сте дељење за групу контаката {sharee}",
    "{actor} has restored the share for contact group {sharee}" : "{actor} је вратио дељење за групу контаката {sharee}",
    "You have restored a share from inquiry {inquiryTitle}" : "Вратили сте дељење у упитник {inquiryTitle}",
    "{actor} has restored a share from inquiry {inquiryTitle}" : "{actor} је вратио дељење у упитник {inquiryTitle}",
    "You have done something indescribable with inquiry {inquiryTitle}" : "Урадили сте нешто неописиво са упитником {inquiryTitle}",
    "{actor} has done something indescribable with inquiry {inquiryTitle}" : "{actor} је урадио нешто неописиво са упитником {inquiryTitle}",
    "You have done something indescribable with this inquiry" : "Урадили сте нешто неописиво са овим упитником",
    "{actor} has done something indescribable with this inquiry" : "{actor} је урадио нешто неописиво са овим упитником",
    "Events happening inside of a <strong>inquiry</strong>" : "Догађаји који се дешавају унутар <strong>упитника</strong>",
    "Someone supported in a <strong>inquiry</strong>" : "Неко је дао подршку у <strong>упитнику</strong>",
    "Participatory democracy app for creating, debating and supporting on community proposals.." : "Апликација учесничке демократије за креирање расправљање и подржавање предлога заједнице.",
    "Agora is a Nextcloud app for participatory democracy. Users can create proposals, debates, petitions or projects, and gather suggestions, comments and supports. Contributions can be organized by location or category, and evolve through customizable workflows." : "Агора је Nextcloud апликација за учесничку демократију. Коринсици могу да креирају предлоге, дебате, петиције или пројекте, као и да прикупе сугестије, коментаре и подршку. Доприноси могу да се организују према локацији или категорији, и могу да се развијају кроз прилагодљиве токове рада.",
    "Add inquiry" : "Додај упитник",
    "\"{inquiryTitle}\" has been successfully created." : "„{inquiryTitle}” је успешно креиран.",
    "Inquiry created" : "Упитник је креиран",
    "Add another inquiry" : "Додај још један упитник",
    "Open inquiry now" : "Отвори упитник сада",
    "New inquiry" : "Нови упитник",
    "Add option" : "Додај ставку",
    "Delete" : "Обриши",
    "_Deleting in {countdown} second_::_Deleting in {countdown} seconds_" : ["Брисање за {countdown} секунду","Брисање за {countdown} секунде","Брисање за {countdown} секунди"],
    "Delete orphaned" : "Обриши заостале",
    "Add some!" : "Додајте неку!",
    "Edit access" : "Уреди приступ",
    "Register" : "Региструј се",
    "Send confirmation mails" : "Пошаљи мејлове потврде",
    "See result" : "Погледајте резултат",
    "Result of sent confirmation mails" : "Резултат послатих мејлова потврде",
    "No valid email address" : "Нема исправне и-мејл адресе",
    "Unknown error" : "Непозната грешка",
    "Reveal them" : "Откриј их",
    "Toggle sidebar" : "Укључи/искључи бочну траку",
    "Maybe" : "Можда",
    "Loading" : "Учитавам",
    "Support status" : "Статус подршке",
    "Click" : "Клик",
    "Previous day" : "Претходни дан",
    "Next day" : "Наредни дан",
    "minus" : "  минус",
    "plus" : "плус",
    "Please wait…" : "Молимо сачекајте…",
    "Loading …" : "Учитавање…",
    "Possible calendar conflicts" : "Могући сукоби календара",
    "Add" : "Додај",
    "You are asked to propose more options." : "Од вас је затражено да предложите још опција.",
    "The suggestion period ends {timeRelative}." : "Период за давање предлога се завршава за {timeRelative}",
    "Propose an option" : "Предложи опцију",
    "This inquiry is an anonymous inquiry for everyone, including the owner. Deanonymizing is disabled for this inquiry." : "Овај упитник је анониман за свакога, па чак и за власника. Са овог упитника не може да се уклони анонимност.",
    "But be aware that your name is not stored in an encrypted or obfuscated way." : "Али имајте на уму да ваше име није сачувано шифровано или замагљено.",
    "This inquiry is an anonymous inquiry for everyone, except for the owner and delegated inquiry administration." : "Овај упитник је анониман за свакога, осим за власника и проглашену администрацију упитника.",
    "Anonymization of this inquiry can be removed at any time by the owner and delegated inquiry administration." : "Власник и проглашена администрација упитника у сваком тренутку могу да уклоне анонимност овог упитника.",
    "This inquiry is closed." : "Овај упитник је затворен.",
    "No further action is possible." : "Нису могуће накнадне акције.",
    "No more voting options are available." : "Нема више доступних опција гласања.",
    "_%n voting option is available._::_%n voting options are available._" : ["Доступна је %n опција гласања.","Доступне су %n опције гласања.","Доступно је %n опција гласања."],
    "This share is locked and allows only read access. Registering is not possible." : "Ово дељење је закључано и можете само да га читате. Регистрација није могућа.",
    "To participate, register with your email address and a name." : "Ако желите да учествујете, региструјте се својом и-мејл адресом и именом.",
    "To participate, register a name and optionally with your email address." : "Ако желите да учествујете, региструјте се својим именом и ако желите и и-мејл адресом.",
    "To participate, register with a name." : "Ако желите да учествујете, региструјте се својом и-мејл адресом и именом.",
    "You have confirmed options. Inform your participants about the result via email." : "Потврдили сте опције. Обавестите ваше учеснике о резултату користећи и-мејл.",
    "Some confirmation messages could not been sent." : "Неке поруке потврде нису могле да се пошаљу.",
    "Messages sent." : "Поруке су послате.",
    "Existing participants will still have access." : "Постојећи учесници још увек имају приступ.",
    "Invite users or allow internal access for all site users." : "Позовите кориснике или дозволите интерни приступ свим корисницима сајта.",
    "Only visible to me" : "Видим само ја",
    "Only visible to {displayName}" : "Види само {displayName}",
    "Error while saving comment" : "Грешка приликом чувања коментара",
    "New comment …" : "Нови коментар…",
    "Confidential with {displayName}" : "У поверењу са {displayName}",
    "Confidential" : "Поверљиво",
    "Error while deleting the comment" : "Грешка приликом брисања коментара",
    "Error while restoring the comment" : "Грешка приликом враћања коментара",
    "Restore comment" : "Враћање коментара",
    "Delete comment" : "Обриши коментар",
    "deleted" : "обрисано",
    "The automatic reminder is sent to all shares via email:" : "И-мејлом је послат аутоматски подсетник свим дељењима:",
    "No reminder is sent:" : "Подсетник се не шаље:",
    "If a share already got a reminder (no matter, if the expiration date got changed)." : "Ако је дељење већ примио подсетник (без обзира, ако је промењен термин истека).",
    "Once enabled, the anonymous setting cannot be reverted anymore." : "Једном када се укључи, подешавање анонимности више не може да се врати на старо.",
    "Cancel" : "Откажи",
    "Ok" : "У реду",
    "Use Autoreminder" : "Користи аутоматски подсетник",
    "Autoreminder information" : "Информацијa аутоматског подсетника",
    "Collapse long descriptions" : "Сажми дугачке описе",
    "Force confidential comments (only visible to you and the author)" : "Форсирај поверљиве коментаре (које видите само ви и аутор)",
    "Force confidential comments (only visible to {displayName} and the author)" : "Форсирај поверљиве коментаре (које виде само {displayName} и аутор)",
    "Hide unavailable Options" : "Сакриј опцијe које нису доступне",
    "Always show results" : "Увек прикажи резултате",
    "Never show results" : "Никад не приказуј резултате",
    "Title" : "Наслов",
    "Enter title" : "Унесите назив",
    "Close" : "Затвори",
    "URL" : "URL",
    "Italic" : "Курзив",
    "Underline" : "Подвучено",
    "Strikethrough" : "Прецртано",
    "Paragraph" : "Пасус",
    "Blockquote" : "Издвојени цитат",
    "Insert image" : "Убаци слику",
    "Code block" : "Блок са изворним кôдом",
    "Undo" : "Опозови",
    "Redo" : "Понови",
    "Medium" : "Средњи",
    "Preview:" : "Преглед:",
    "Save" : "Сачувај",
    "Official" : "Званичне",
    "Location" : "Локација",
    "Category" : "Категорија",
    "Projects" : "Пројекти",
    "Suggestions" : "Предлози",
    "All categories" : "Све категорије",
    "Filters" : "Филтери",
    "Clear all" : "Очисти све",
    "Type" : "Тип",
    "Comments" : "Коментари",
    "Archived" : "Архивирано",
    "Unpublished" : "Необјављено",
    "never" : "никад",
    "Results are always hidden" : "Резултати су увек скривени",
    "Results are visible" : "Резултати су видљиви",
    "Created {dateRelative}" : "Креирано {dateRelative}",
    "Closing: {dateRelative}" : "Затвара се за: {dateRelative}",
    "Time zone: {timezoneString}" : "Временска зона: {timezoneString}",
    "_%n Participant_::_%n Participants_" : ["%n учесник","%n учесника","%n учесника"],
    "_%n option_::_%n options_" : ["%n опција","%n опције","%n опција"],
    "No description provided" : "Није дат опис",
    "Archived {relativeTime}" : "Архивирано је пре {relativeTime}",
    "Started {relativeTime} from {ownerName}" : "Покренуо {ownerName} пре {relativeTime}",
    "{count} participants" : "{count} учесника",
    "Expiration" : "Истиче",
    "Created on {date}" : "Креирано дана {date}",
    "Back" : "Назад",
    "Create new group" : "Креирај нову групу",
    "New group name" : "Име нове групе",
    "Sort by title" : "Поређај по наслову",
    "Sort by last interaction" : "Поређај по последњем времену интеракције",
    "Sort by creation date" : "Поређај по датуму креирања",
    "Sort by expiration date" : "Поређај по датуму истека",
    "Sort by owner name" : "Поређај по имену власника",
    "Direction" : "Смер",
    "Descending" : "опадајуће",
    "Ascending" : "растуће",
    "Home" : "Почетна",
    "Enter a description" : "Унесите опис",
    "Choose a description for the overview page" : "Изаберите опис за страницу прегледа",
    "Choose a brief title for the navigation bar and the slug" : "Изаберите кратак наслов за линију навигације и пужа",
    "Enter extended title" : "Унесите проширени наслов",
    "Optionally choose a more meaningful title for the overview page" : "По жељи, за страницу прегледа изаберите наслов са више смисла",
    "Note: Based on the name change, the URL has also changed" : "Напомена: сагласно са променом имена, променила се и URL адреса",
    "Name" : "Име",
    "Extended title" : "Проширени наслов",
    "Description" : "Опис",
    "OK" : "OK",
    "Add date option" : "Додај опцију датума",
    "Adding options is disabled" : "Искључено је додавање опција",
    "Existing options" : "Постојеће опције",
    "Select the user to transfer the ownership to" : "Изаберите корисника коме желите да пренесете власништво.",
    "Clone to option sequence" : "Секвенца клонирања опција",
    "Create a sequence of date options starting with {dateOption}." : "Направи секвенцу датума почевши од {dateOption}.",
    "Step unit" : "Јединица корака",
    "Step width" : "Ширина корака",
    "Amount" : "Количина",
    "Restore option" : "Опција враћања",
    "Delete option" : "Обриши опцију",
    "{displayName}'s proposal" : "Предлог корисника {displayName}",
    "Clone option" : "Опције клонирања",
    "Unconfirm option" : "Склони опцију гласања",
    "Confirm option" : "Потврди опцију гласања",
    "Sort" : "Разврставање",
    "{optionText} added" : "Додата је {optionText}",
    "{optionText} already exists" : "{optionText} већ постоји",
    "Error adding {optionText}" : "Грешка приликом додавања {optionText}",
    "Add options list (one option per line)" : "Додајте листу опција (једна по линији)",
    "Paste option list" : "Налепи листу опција",
    "Options added" : "Опције су додате",
    "Error adding options" : "Грешка приликом додавања опција",
    "Create multiple options at once" : "Креирај више опција одједном",
    "Each line creates a new option. Duplicates will get skipped without warning." : "Свака линија креира нову опцију. Дупликати ће се прескочити без упозорења.",
    "Checking name …" : "Проверава се име...",
    "A name is required." : "Потребно је име.",
    "The name {username} is invalid or reserved." : "Име {username} је неисправно или резервисано.",
    "Checking email address …" : "Проверавам адресу е-поште…",
    "An email address is required." : "Потребна је и-мејл адреса.",
    "Invalid email address." : "Неисправна и-мејл адреса.",
    "You will receive your personal link after clicking \"OK\"." : "Када кликнете на „ОК” добићете свој лични линк.",
    "Enter your email address to get your personal access link." : "Унесите своју и-мејл адресу да бисте добили свој лични линк.",
    "Email could not be sent to {emailAddress}" : "Не може да се пошаље и-мејл на {emailAddress}",
    "Guest participants" : "Гостујући учесници",
    "Enter your name or a nickname" : "Унесите своје име или надимак",
    "Email address (mandatory)" : "И-мејл адреса (обавезно)",
    "Email address (optional)" : "И-мејл адреса (није обавезно)",
    "Remember me for 30 days" : "Запамти ме за 30 дана",
    "Privacy policy" : "Смернице приватности",
    "Registered accounts" : "Регистровани налози",
    "Login" : "Пријава",
    "You can also log in and participate with your regular account." : "Такође можете да се пријавите и да учествујете користећи свој обични налог.",
    "Otherwise participate as a guest participant." : "У супротном можете да учествујете као гостујући учесник.",
    "Enable the tracking of activities with the Activities app" : "Укључи праћење активности апликацијом Активности",
    "Categories" : "Категорије",
    "Locations" : "Локације",
    "Edit" : "Измени",
    "Enable the usage of the combo view globally" : "Укључи употребу комбинованог погледа свима",
    "Enable only for the following groups" : "Уљкучи само за следеће групе",
    "Leave empty to disable globally" : "Оставите празно да искључите за све",
    "Add terms links also to the email footer" : "Такође додај линкове на услове у подножје и-мејла",
    "Additional email disclaimer" : "Додатни и-мејл одрицања од одговорности",
    "Preview" : "Преглед",
    "Collectives" : "Колективи",
    "Enabled" : "укључено",
    "Settings" : "Поставке",
    "Allow comments" : "Дозволи коментаре",
    "Run autoreminder" : "Покрени аутоматски подсетник",
    "Run janitor" : "Покрени домара",
    "Run notification" : "Покрени обавештење",
    "Autoreminder started" : "Аутоматски подсетник је покренут",
    "Autoreminder failed" : "Аутоматски подсетник није успео",
    "Janitor started" : "Домар је покренут",
    "Janitor failed" : "Домар није успео да се покрене",
    "Notification started" : "Обавештење је покренуто",
    "Notification failed" : "Обавештење није успело да се изврши",
    "Please understand, that the jobs were defined as asynchronous jobs by intention." : "Послови су намерно дефинсани као асинхронии послови.",
    "Only use them, if it is absolutely neccessary (i.error. your cron does not work properly) or for testing." : "Користите их само ако је то апсолутно неопходно (у случају да ваш cron не функционише како треба) или само за тестирање.",
    "Starting the jobs does not mean, that the rules for these actions are overridden." : "Покретање посла не значи да се правила за ове акције преиначавају.",
    "Each job can only be run once. If you want to rerun them, you have to refresh the page." : "Сваки посао може да се покрене само једном. Ако желите поново да их покренете, морате поново да учитате страницу.",
    "If you want to see the result, please check the logs." : "Ако желите да видите резултат, молимо вас да погледате дневнике.",
    "Enter the URL of your privacy policy" : "Унесите URL ваше политике приватности",
    "Enter the URL of your legal notice" : "Унесите URL ваше правне напомене",
    "Privacy policy link:" : "Линк на полису приватности:",
    "Legal terms link:" : "Линк на правне услове:",
    "Label" : "Ознака",
    "Show email addresses of internal accounts" : "Приказ и-мејл адресе интерних налога",
    "Show only to members of the following groups" : "Прикажи само члановима следећих група",
    "Leave empty to disable globally." : "Оставите празно да искључите за све.",
    "Enable unrestricted owners globally" : "Укључи неограничене власнике глобално",
    "Effects on restricted owners:" : "Последице за ограничене власнике:",
    "Use calendar lookup for conflicting calendar events" : "Користи претрагу календара за догађаје у конфликту",
    "Select the calendars to use for lookup." : "Изаберите календаре који треба да се претраже.",
    "Specify in which period (in hours) before the option existing appointments should be included in the search results." : "Наведите период (у сатима) пре опције у којем би требало да се постојећи састанци укључе у резултате претраге.",
    "Specify in which period (in hours) after the option existing appointments should be included in the search results." : "Наведите период (у сатима) након опције у којем би требало да се постојећи састанци укључе у резултате претраге.",
    "The style settings are still experimental!" : "Подешавања стила су још увек експериментална!",
    "Use alternative styling for the comments sidebar" : "Користите алтернативни стил за бочну траку за коментаре",
    "Personal preferences" : "Лична подешавања",
    "Styles" : "Стилови",
    "Share label" : "Ознака дељења",
    "Internal access" : "Интерни приступ",
    "Resend invitation mail" : "Поново пошаљи позивницу",
    "Send invitation mail" : "Пошаљи позивницу",
    "Error sending invitation" : "Грешка приликом слања позивнице",
    "Invitation sent to {displayName} ({emailAddress})" : "Позивница је постата кориснику {displayName} ({emailAddress})",
    "Error sending invitation to {displayName} ({emailAddress})" : "Грешка приликом слања позивнице кориснику {displayName} ({emailAddress})",
    "Resolve group into individual invitations" : "Разреши групу у појединачне позивнице",
    "Resolving of {name} is not possible. The contacts app is not enabled." : "Није могуће разрешење {name}. Апликација контакти није укључена.",
    "Resolving of {name} is not possible. The circles app is not enabled." : "Није могуће разрешење {name}. Апликација кругови није укључена.",
    "Error resolving {name}." : "Грешка приликом разрешавања {name}",
    "Copy link to clipboard" : "Копирај везу у оставу",
    "Link copied to clipboard" : "Веза копирана у оставу",
    "Error while copying link to clipboard" : "Грешка приликом копирања везе у оставу",
    "Show QR code" : "Прикажи QR кôд",
    "Unlock share" : "Откључај дељење",
    "Lock share" : "Закључај дељење",
    "Error while changing lock status of share {displayName}" : "Дошло је до грешке приликом промене статуса закључаности дељења {displayName}",
    "Restore share" : "Поврати дељење",
    "Delete share" : "Обриши дељење",
    "Error while changing deleted status of share {displayName}" : "Дошло је до грешке приликом промене статуса обрисаности дељења {displayName}",
    "Options for the registration dialog" : "Опције за дијалог регистрације",
    "Email address is optional" : "И-мејл адреса није обавезна",
    "Email address is mandatory" : "И-мејл адреса је обавезна",
    "Do not ask for an email address" : "Не тражи и-мејл адресу",
    "Add a new public link" : "Додај нови јавни линк",
    "Error adding public link" : "Грешка приликом додавања јавног линка",
    "Shares" : "Дељења",
    "Error while adding share" : "Грешка приликом додавања дељења",
    "Add shares" : "Додај дељења",
    "Type to add an individual share" : "Куцајте да додате појединачно дељење",
    "Locked shares (read only access)" : "Закључана дељења (приступ само за читање)",
    "Unsent invitations" : "Непослате позивнице",
    "Resolve and send all invitations" : "Разреши и пошаљи све позивнице",
    "No activity yet" : "Још нема активности",
    "Failed to upload {file}" : "{file} није успео да се отпреми",
    "No attachments" : "Без прилога",
    "Delete attachment" : "Обриши прилог",
    "No comments" : "Нема коментара",
    "Be the first." : "Будите први.",
    "Receive notification email on activity to {emailAddress}" : "Примај и-мејл обавештења о активностима на {emailAddress}",
    "Receive notification email on activity" : "Примај обавештења о активностима преко е-поште",
    "Change name" : "Промени име",
    "Name changed." : "Име је промењено.",
    "Error changing name." : "Грешка приликом промене имена.",
    "Edit Email Address" : "Уреди и-мејл адресу",
    "Email address {emailAddress} saved." : "Сачувана је и-мејл адреса {emailAddress}.",
    "Error saving email address {emailAddress}" : "Грешка приликом чувања и-мејл адресе {emailAddress}",
    "(deleted)" : "(обрисано)",
    "(locked)" : "(закључано)",
    "Administrative rights granted" : "Одобрена су административна права",
    "Anonymized participant" : "Анонимизирани учесник",
    "Deleted participant" : "Обрисани учесник",
    "Token: {token}" : "Жетон: {token}",
    "Public link: {token}" : "Јавни линк: {token}",
    "Public link" : "Јавна веза",
    "Resolve this group first!" : "Најпре разреши ову групу!",
    "Email address deleted." : "И-мејл адреса је обрисана.",
    "Error deleting email address {emailAddress}" : "Грешка приликом брисања и-мејл адресе {emailAddress}",
    "Invitation resent to {emailAddress}" : "Позивница је поново послата на {emailAddress}",
    "Mail could not be resent to {emailAddress}" : "Мејл није могао поново да се пошаље на {emailAddress}",
    "Edit email address" : "Уреди и-мејл адресу",
    "Switch to table view" : "Пребаци на поглед табеле",
    "Switch to list view" : "Пређи на поглед листе",
    "Switch to date order" : "Пребаци се на редослед датума",
    "Switch to original order" : "Пребаци се на оригинални фолдер",
    "Switch to ranked order" : "Пребаци се на редослед по рангу",
    "Copy your personal link to clipboard" : "Копирај ваш лични линк у клипборд",
    "Get your personal link per mail" : "Примите свој лични линк на и-мејл",
    "Subscribe to notifications" : "Претплати се на обавештења",
    "Remove email address" : "Уклони и-мејл адресу",
    "Copy list of email addresses to clipboard" : "Копирај списак е-мејл адреса у оставу",
    "Logout as {name} (delete cookie)" : "Одјави се као {name} (обриши колачић)",
    "Type to start searching …" : "Куцајте да почнете претрагу…",
    "Select users" : "Изаберите кориснике",
    "Share settings" : "Подешавања дељења",
    "Change share settings globally (for all accounts)" : "Промени подешавања дељења за све (налоге)",
    "Other settings" : "Остала подешавања",
    "Enable or disable individual features." : "Укључује или искључује поједине функције.",
    "Performance settings" : "Подешавања перформанси",
    "If you are experiencing connection problems, change how auto updates are retrieved." : "Ако имете проблеме са везом, промените начин на који се примају аутоматска ажурирања.",
    "Email options" : "Опције за и-мејл",
    "Add links to legal terms, if they exist and add an optional disclaimer to emails." : "Додавање линкова на правне услове, ако постоје, и додавање необавезног одрицања од одговорности у и-мејлове.",
    "Job control" : "Контрола посла",
    "Manually start backgropund jobs, independent from the cron schedule." : "Ручно покренути позадински послови који не зависе од cron распореда.",
    "Error setting dashboard list" : "Грешка приликом постављања листе контролне табле",
    "Contact the owner, if you think that this is wrong." : "Контактирајте власника ако сматрате да ово није у реду.",
    "Checking access…" : "Проверава се приступ…",
    "Almost ready…" : "Скоро па готово…",
    "Do not go away…" : "Не напуштајте страницу",
    "Please be patient…" : "Молимо да будете стрпљиви…",
    "Add one or change category!" : "Додајте неко или промените категорију!",
    "Click here to load more" : "Кликните овде да се учита још",
    "Fetching configuration…" : "Преузима се конфигурација…",
    "Collecting elements…" : "Прикупљају се елементи…",
    "Closing {relativeExpirationTime}" : "Затвара се за {relativeExpirationTime}",
    "View all" : "Прикажи све",
    "participated" : "учествовао",
    "closed" : "затворено",
    "By:" : "Корисник:",
    "Details" : "Детаљи",
    "Attachments" : "Прилози",
    "Sharing" : "Дељење",
    "Activity" : "Активност",
    "Configuration" : "Конфигурација",
    "Experimental styles" : "Експериментални стилови",
    "Some visual styling options." : "Неке опције за промену визуелног стила.",
    "Minute" : "Минут",
    "Hour" : "Сат",
    "Day" : "Дан",
    "Week" : "Недеља",
    "Month" : "Месец",
    "Year" : "Година",
    "Created" : "Направљен",
    "Access" : "Приступ",
    "Owner" : "Власник",
    "Expire" : "Истиче",
    "Last interaction" : "Последња интеракција",
    "Relevant" : "Релевантно",
    "Participated" : "Учествовао/ла",
    "Archive" : "Архива",
    "Administration" : "Администрација",
    "Administrative access" : "Административни приступ",
    "Title must not be empty!" : "Наслов не сме бити празан!",
    "Invalid Group ID" : "Неисправан ID групе"
},
"nplurals=3; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<10 || n%100>=20) ? 1 : 2);");
