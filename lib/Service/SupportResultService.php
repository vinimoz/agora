...
class SupportResultService
{
    ...
    /**
     * Get all support results for a specific inquiry group
     *
     * This returns all stored results where target_type = 'inquirygroup' and target_id = $inquiryGroupId.
     * Results may exist for different support engines (including null for informal/deliberative).
     *
     * @param int $inquiryGroupId The inquiry group ID
     * @return SupportResult[] Array of SupportResult entities
     */
    public function getResultsByInquiryGroup(int $inquiryGroupId): array
    {
        $this->logger->debug('Getting results by inquiry group id', ['inquiryGroupId' => $inquiryGroupId]);
        $results = $this->resultMapper->findResultsByTarget('inquirygroup', $inquiryGroupId);
