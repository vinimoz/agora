...
export const useInquiryGroupStore = defineStore('inquiryGroup', () => {
  ...
  const family = computed(() => {
    return typeTemplate.value?.family
  })
  ...
  const displayArchitecture = computed(() => {
    return ui.value?.display_architecture
  })
  ...
  const experience = computed(() => {
    return ui.value?.experience || 'dashboard'
  })
  ...
  const layout = computed(() => {
    return ui.value?.layout || { type: 'grid', columns: 1, rows: 1, responsive: true }
  })
  ...
  const allowedInquiryTypes = computed(() => {
    return typeTemplate.value?.allowedInquiryTypes || 
  ...
  const isRoot = computed(() => {
    return typeTemplate.value?.is_root || false
  })
  ...
  const actions = computed(() => {
    return typeTemplate.value?.actions || []
  })
   ...
   */
  const icon = computed(() => {
    return typeTemplate.value?.icon || 'Folder'
  })
  ...
  const label = computed(() => {
    return typeTemplate.value?.label || type.value || 'Group'
  })
  ...
  const templateDescription = computed(() => {
    return typeTemplate.value?.description || ''
  })
  ...
  const rules = computed(() => {
    return typeTemplate.value?.rules || {}
  })

  /**
   * Get allowed response from template
   */
  const allowedResponse = computed(() => {
    return typeTemplate.value?.allowed_response || []
  })
  ...
  const fields = computed(() => {
    return typeTemplate.value?.fields || []
  })
