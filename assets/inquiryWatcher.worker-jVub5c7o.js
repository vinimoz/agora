...
(function() {
      ...
      if (value > max || value < min) {
        ...
        let range;
        {
          if (min === 0 || min === BigInt(0)) {
            range = ">= 0".concat(n, " and < 2").concat(n, " ** ").concat((byteLength3 + 1) * 8).concat(n);
          } else {
            range = ">= -(2".concat(n, " ** ").concat((byteLength3 + 1) * 8 - 1).concat(n, ") and < 2 ** ") + "".concat((byteLength3 + 1) * 8 - 1).concat(n);
          }
        }
  ...
  self.onmessage = async (props) => {
    ...
    const run = async () => {
      var _a;
      try {
        const endPoint = token ? "s/".concat(token, "/watch") : "inquiry/".concat(inquiryId, "/watch");
        const response = await http.get(endPoint, { params: { offset: lastUpdated } });
        consecutiveErrors = 0;
        if (response.status === 200 && ((_a = response.data.updates) == null ? void 0 : _a.length)) {
        ...
        sendMessage({
          ...
          mode: updateType,
          interval,
          message: "[Worker] Request failed (".concat(consecutiveErrors, "/").concat(MAX_ERRORS, ")")
        });
        if (consecutiveErrors >= MAX_ERRORS) {
          sendMessage({
            ...
            mode: updateType,
            interval,
            message: "[Worker] Stopping after ".concat(MAX_ERRORS, " consecutive errors")
          });
          self.close();
        }
...
//# sourceMappingURL=inquiryWatcher.worker-jVub5c7o.js.map
